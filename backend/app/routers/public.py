...
def nearby_bank_branches(
    ...
    # location_precision), so multiple branches in the same city (or in two
    # differently-spelled entries for the same real place) land on the exact
    # same distance. Showing five identical distances reads as broken even
    # though each number is technically correct, so only the first (nearest-
    # ranked) branch at each distinct distance is kept — the rest are real
    # branches too, just not shown here since they wouldn't add new distance
    # information.
    seen_distances = set()
    deduped = []
    for r in ranked:
        if r["distance_km"] in seen_distances:
            continue
